 ...
 * @LastEditTime: 2020-10-09 15:49:38
 */
#pragma once

#define G3CAN_ENABLE 1
...
class xp_g3_ros_core {
 private:
  // control
  control_msgs::Vehicle_control xpg3_control;
  control_msgs::Vehicle_status xpg3_status;
  control_msgs::Pathpoint xpg3_pose;
  ...
  // class
  icvCarlaControl zz;             // lat
  LonControl xpg3_loncontroller;  // lon
  // update input data
  ...
  void publish_control_instruction(float out_acc, float out_steer,
                                   int brake_hold);
  void pubVechilepose2rviz();
  void VehicelStatusSend();
  // trajectory analyse ,will rm to analyse.h.9.17
  float acc_boundary = 0;
  float jerk_boundary = 0;
  void get_accleration_jerk(
      const zzz_planning_msgs::DecisionTrajectory &msg,
      int desire_number, float *limit_acc, float *limit_jerk);
