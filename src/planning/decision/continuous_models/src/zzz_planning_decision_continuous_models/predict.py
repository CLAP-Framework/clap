...
class JunctionPredict():
    def __init__(self, dynamic_map, considered_obs_num, maxt, dt, robot_radius, radius_speed_ratio, move_gap, ego_speed, last_loop_obstacles):
        self.considered_obs_num = considered_obs_num
        self.maxt = maxt
        self.dt = dt
        self.check_radius = robot_radius + radius_speed_ratio * ego_speed
        self.move_gap = move_gap
        self.last_loop_obstacles = last_loop_obstacles
        ...
        self.obs = self.found_closest_obstacles()
        self.obs_paths = self.ctrv_prediction_obstacle(self.obs, self.maxt, self.dt)
    ...
    def found_closest_obstacles(self):
        ...
        for obs in self.dynamic_map.jmap.obstacles: 
            ...
            one_obs = (obs.state.pose.pose.position.x , obs.state.pose.pose.position.y , obs.state.twist.twist.linear.x ,
                     obs.state.twist.twist.linear.y , p4 , obs_ffstate.s , -obs_ffstate.d , obs_ffstate.vs, obs_ffstate.vd, 
                     obs.state.accel.accel.linear.x, obs.state.accel.accel.linear.y, obs_yaw, classid, obs.uid)
            obs_tuples.append(one_obs)
    ...
    def ctrv_prediction_obstacle(self, ob, max_prediction_time, delta_t): # we should do prediciton in driving space
        ...
        for one_ob in ob:
            if one_ob[12] != 2: # vehicle
                ...
                velocity_module = (one_ob[2]**2 + one_ob[3]**2)**0.5 # we lose direction infomation of vehicle here
                if np.dot(np.array([cos(one_ob[11]), sin(one_ob[11])]),np.array([one_ob[2], one_ob[3]])) < 0:
                    velocity_module = - velocity_module
                start_state = np.array([one_ob[0], one_ob[1], velocity_module, one_ob[11], self.get_yaw_rate(one_ob)])
                last_state = start_state
                for t in obsp_front.t:
                    state_change_rate = np.array([(last_state[2]/last_state[4])*(sin(last_state[3] + last_state[4] * delta_t) - sin(last_state[3])),
                                        (last_state[2] / last_state[4]) * (- cos(last_state[3] + last_state[4] * delta_t) + cos(last_state[3])),
                                        0,
                                        last_state[4] * delta_t,
                                        0
                                        ])
                    last_state = last_state + state_change_rate

                    obsp_front.x.append(last_state[0] + math.cos(last_state[3]) * self.move_gap)
                    obsp_front.y.append(last_state[1] + math.sin(last_state[3]) * self.move_gap)
                    obsp_back.x.append(last_state[0] - math.cos(last_state[3]) * self.move_gap)
                    obsp_back.y.append(last_state[1] - math.sin(last_state[3]) * self.move_gap)
    ...
    def get_yaw_rate(self, obs):
        for last_obs in self.last_loop_obstacles:
            if obs[13] == last_obs.uid:
                yaw_rate = (obs[11] - get_yaw(last_obs.state))/0.2 # 5hz
                print("!!!!!!!!!!!!keep tracking a known vehicle ")
                return yaw_rate if yaw_rate != 0 else 0.0001
        return 0.0001
