...
private:
    ...
    float feedforward_K = 0; //  Curvature feed forward gravity （pre_K+feedforward_K=1）
    ...
    void Vehiclemodeupdate1(Pathpoint coordinate, float delta_f, float T, float *x_vehicle, float *y_vehicle, float *heading);
    void Vehiclemodeupdate2(Pathpoint coordinate, float delta_f, float T, float *x_vehicle, float *y_vehicle, float *heading);
    ...
    //跟踪方案二
    double ControlStr2(float _vehicle_x, float _vehicle_y, float _vehicle_heading_theta, float _vehicle_v);
    ...
    double CalculateCur(geometry_msgs::Point P1, geometry_msgs::Point P2, geometry_msgs::Point P3);                //三点计算曲率
    double smoothfilter(double param, int smoothsize);                                                             //多点平滑滤波器参数为平滑量，平滑点数
    geometry_msgs::Point calcRelativeCoordinate(geometry_msgs::Point point_msg, geometry_msgs::Pose current_pose); //坐标转换
    ...
    bool Pathlength(double *pathlength); //计算路径长度和每两个点的间距,判断是否正常,是否需要插值.
...
public:
    ...
    {
        //TODO::update vehicle chassis feedback data  coordinate(x,y,theta)(m,m,deg),speed(m/s),steerangle_rec(deg)
        Current_Point.x = _x;
        Current_Point.y = _y;
