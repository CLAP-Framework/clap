...
int main(int argc, char **argv)
{
  ...
  signal(SIGINT, MySigintHandler);
    ...
    if(callback_imu_flag&callback_gps_flag)
    {
      pathdeal();
      float steer1=D_PID_realize();
      float steer2=A_PID_realize();
      float steer_send=steer1+steer2;
      ...
      else
      {
          ...
          ctrmsg.AutoMode=1;
          ctrmsg.TarSpeedReq=SpeedReq=target_speed; 
...
float  D_PID_realize(){
  // D_pid.SetSteerAngle=SetSteerAngle;
  // D_pid.cte_d=D_pid.SetSteerAngle-D_pid.ActualSteerAngle;
...
{    
  ...
  // cout<<(Waypoints[plo_i].x)<<"dd "<<Current_Point.x<<endl;
  if(sig_d>=0)    
  {
    if(sig_d>0)
    {
  ...
  // cout<<"sgn "<<sig_d<<endl;
  cte_D=ctd*sgn;    
  int i_pre=int(15+(RRWheelSpd-10));//预瞄点设置
  cte_A=0-(fmod(((Waypoints[plo_i+i_pre].theta-Current_Point.theta)/180.0*PI+PI),(2*PI))-PI)/PI*180.0;
