...
/*void yuyv2rgb_avx(char* YUV,  char* RGB, int NumPixels) {
 
  unsigned char* yuv;
  unsigned char* rgb;

  uint8_t* yuv_offset = (unsigned char*)YUV;
  uint8_t* rgb_offset = (unsigned char*)RGB;

  yuyv2rgb_avx(yuv, rgb, NumPixels);
}*/


void yuyv2rgb_avx(unsigned char* YUV, unsigned char* RGB, int NumPixels) {
  assert(NumPixels == (1920 * 1020));
