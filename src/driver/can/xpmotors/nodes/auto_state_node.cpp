...
//	  pid_t pid = getpid();		
//	  ROS_INFO("xpmotors-auto_state_node process id is: %d.", static_cast<int>(pid));	  
	  //node_status_pub_ptr->CHECK_CPU_USAGE("xpmotors-auto_state_node", pid, 1, 2, 5, "xpmotors-auto_state_node CPU usage high.");
	  //node_status_pub_ptr->CHECK_MEMORY_USAGE("xpmotors-auto_state_node", pid, 0.1, 0.2, 0.5, "xpmotors-auto_state_node memory usage high.");
//	  node_status_pub_ptr->CHECK_FD_NUM("xpmotors-auto_state_node", pid,15, 20, 30, "xpmotors-auto_state_node file description number much.");
...
//			node_status_pub_ptr->NODE_ACTIVATE();
 // 			node_status_pub_ptr->CHECK_RATE("/xp/auto_state_ex", 40, 25, 5, "topic '/xp/auto_state_ex' publish rate low.");
...
//			node_status_pub_ptr->NODE_ACTIVATE();
//  			node_status_pub_ptr->CHECK_RATE("/xp/auto_state", 40, 25, 5, "topic '/xp/auto_state' publish rate low.");

			pubState.publish(aState);
			break;
		}
		default:
			//ROS_WARN("xpmotors - frame ID: %x is out of range.", Tframe.frame.id);
			break;
