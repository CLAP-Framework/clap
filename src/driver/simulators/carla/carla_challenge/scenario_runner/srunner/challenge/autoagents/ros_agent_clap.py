...
class RosAgentClap(AutonomousAgent):
    ...
    def setup(self, path_to_conf_file):
        ...
        print("Path '{}' defined by TEAM_CODE_ROOT VALIDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD".format(team_code_path))
    ...
    def get_object(self, target_vehicle_id, target_vehicle):
        ...
        # Transform the direction like in manual.py

        x = vehicle_target.bbox.pose.pose.orientation.x
        y = vehicle_target.bbox.pose.pose.orientation.y
        z = vehicle_target.bbox.pose.pose.orientation.z
        w = vehicle_target.bbox.pose.pose.orientation.w

        rotation_mat = numpy.array([[1 - 2 * y * y - 2 * z * z, 2 * x * y + 2 * w * z, 2 * x * z - 2 * w * y],
                                 [2 * x * y - 2 * w * z, 1 - 2 * x * x - 2 * z * z, 2 * y * z + 2 * w * x],
                                 [2 * x * z + 2 * w * y, 2 * y * z - 2 * w * x, 1 - 2 * x * x - 2 * y * y]])
        rotation_mat_inverse = numpy.linalg.inv(rotation_mat)
        vel_self = numpy.array([[vehicle_target.twist.twist.linear.x], [vehicle_target.twist.twist.linear.y],
                             [vehicle_target.twist.twist.linear.z]])
        vel_world = numpy.matmul(rotation_mat_inverse, vel_self)
        vehicle_target.twist.twist.linear.x = vel_world[0]
        vehicle_target.twist.twist.linear.y = vel_world[1]
        vehicle_target.twist.twist.linear.z = vel_world[2]
        # END
