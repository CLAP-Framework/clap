...
from srunner.scenarios.follow_leading_vehicle import AheadObstacle
...
from srunner.challenge.utils.define_and_config import Common_Define, Common_Config
from srunner.challenge.utils.draw_routes import DrawRoute
from srunner.challenge.utils.draw_velocity import DrawVelocity
from srunner.challenge.utils.draw_excel import DrawExcel, ResultStaticThread
...
class ChallengeEvaluator(object):
    ...
    def __init__(self, args):
        ...
        if args.debug > 0:
            repetitions = Common_Define.repetition_num
        ...
        # Tunable parameters
        self.client_timeout = 300.0  # in seconds
        self.wait_for_world = 200.0  # in seconds
        ...
        Common_Define.first_path = "z_record_file/" + str(now_time)+ "/"
        isExist = os.path.exists( Common_Define.first_path )
        if not isExist:
            os.makedirs( Common_Define.first_path )

        self._velocity_filename = ""
        self._vehicle_location_filename = ""
        self._ref_waypoint_filename = ""

        self._town_name = ''
        self._route_id = ''
        self._route_length = ''
        # self._scenario_type = ''
        Common_Define.left_turn_num = 0
        Common_Define.right_turn_num = 0
        # self._collision_static_num = 0
        # self._collision_vehicle_num = 0
        # self._collision_pedestrian_num = 0
        # self._red_light_num = 0
        # self._wrong_way_num = 0
        # self._route_deviation_num = 0
        # self._sidewalk_invasion_num = 0
        # self._stop_infraction_num = 0
        
        

        # self._route_length_filename = Common_Define.first_path + "route_length_file.txt"
        # self.penalty_file_name = Common_Define.first_path + "penalty_file.txt"
        Common_Define.COLLISION_STATIC_NUM = 0
        Common_Define.COLLISION_VEHICLE_NUM = 0
        Common_Define.COLLISION_PEDESTRIAN_NUM = 0
        Common_Define.TRAFFIC_LIGHT_NUM = 0
        Common_Define.WRONG_WAY_NUM = 0
        Common_Define.ROUTE_DEVIATION = 0
        Common_Define.SIDEWALK_INVASION_NUM = 0
        Common_Define.STOP_NUM = 0
    ...
    def draw_waypoints(self, waypoints, turn_positions_and_labels, vertical_shift, persistency=-1):
        ...
        for start, end, conditions in turn_positions_and_labels:
            ...
            if conditions == RoadOption.LEFT:  # Yellow
                ...
                Common_Define.left_turn_num += 1
            elif conditions == RoadOption.RIGHT:  # Cyan
                color = carla.Color(0, 255, 255)
                Common_Define.right_turn_num += 1
    ...
    def estimate_route_timeout(self, route):
        ...
        print("#### *** #### route_length is {} m".format(route_length))
        self._route_length = str(route_length)
        now_time = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        # with open( self._route_length_filename, 'a') as route_length_file_object:
        #     # route_length_file_object.write(str(now_time) + " " + "\n")
        #     route_length_file_object.write(str(route_length) + "\n")
    ...
    def run_route(self, trajectory, no_master=False):
        ...
        print(" ****** no_master self.route_is_running() {},{}".format(no_master,self.route_is_running()))
        while no_master or self.route_is_running():
            # update all scenarios
            # print("before{}".format(self.list_scenarios))
            GameTime.on_carla_tick(self.timestamp)
            # print("after{}".format(self.list_scenarios))
            CarlaDataProvider.on_carla_tick()
            # update all scenarios
            # rospy.logerr("!!!!!!!!#######self.list_scenarios is")
            ego_action = self.agent_instance()
            # rospy.loginfo("#######JJJJJJ  is {}".format(ego_action))
            ...
            # temp_test = self.list_scenarios[-1]._get_velocity()
            # print(temp_test)
            # ego vehicle acts
            ...
            # print("KKKKKKKKKKKKKKKKKKKKKKKKKKKKno_master self.route_is_running() {},{}".format(no_master,self.route_is_running()))
        ...
        excel_value_list = [Common_Define.excel_rows, self._town_name+'_'+self._route_id, self._route_length, Common_Define.left_turn_num, \
        Common_Define.right_turn_num, Common_Define.COLLISION_STATIC_NUM, Common_Define.COLLISION_VEHICLE_NUM, Common_Define.COLLISION_VEHICLE_NUM, Common_Define.TRAFFIC_LIGHT_NUM, \
        Common_Define.WRONG_WAY_NUM, Common_Define.ROUTE_DEVIATION, Common_Define.SIDEWALK_INVASION_NUM, Common_Define.STOP_NUM, Common_Define.scenario_type]
        result_static_thread = ResultStaticThread(excel_value_list)
        result_static_thread.daemon = True
        result_static_thread.start()
        # drawExcel = DrawExcel()
        # drawExcel.write_excel_xls_append(excel_value_list)
    ...
    def compute_current_statistics(self):
        ...
        for event in list_traffic_events:
            if event.get_type() == TrafficEventType.COLLISION_STATIC:
                ...
                Common_Define.COLLISION_STATIC_NUM += 1
                ...
                Common_Define.COLLISION_VEHICLE_NUM += 1
                msg = event.get_message()
                if msg:
                    list_collisions.append(event.get_message())
                ...
                Common_Define.COLLISION_PEDESTRIAN_NUM += 1
                msg = event.get_message()
                if msg:
                    list_collisions.append(event.get_message())
                ...
                Common_Define.TRAFFIC_LIGHT_NUM += 1
                msg = event.get_message()
                if msg:
                ...
                msg = event.get_message()
                Common_Define.ROUTE_DEVIATION += 1
                if msg:
                ...
                Common_Define.SIDEWALK_INVASION_NUM += 1
                msg = event.get_message()
                if msg:
        ...
        # with open(self.penalty_file_name, 'w') as file_object:
        #     file_object.write("COLLISION_STATIC_NUM  is "+ str(self._COLLISION_STATIC_NUM) +"\n")
        #     file_object.write("COLLISION_VEHICLE_NUM  is "+ str(self._COLLISION_VEHICLE_NUM) +"\n")
        #     file_object.write("COLLISION_PEDESTRIAN_NUM  is "+ str(self._COLLISION_PEDESTRIAN_NUM) +"\n")
        #     file_object.write("TRAFFIC_LIGHT_NUM  is "+ str(self._TRAFFIC_LIGHT_NUM) +"\n")
        #     file_object.write("WRONG_WAY_NUM  is "+ str(self._WRONG_WAY_NUM) +"\n")
        #     file_object.write("SIDEWALK_INVASION_NUM  is "+ str(self._SIDEWALK_INVASION_NUM) +"\n")
        #     file_object.write("STOP_NUM  is "+ str(self._STOP_NUM) +"\n")
    ...
    def load_environment_and_run(self, args, world_annotations, route_description):
        ...
        potential_scenarios_definitions, _ = parser.scan_route_for_scenarios(_route_description,
                                                                             world_annotations)
        # print("#######  AAAA  {}".format(potential_scenarios_definitions))
        ...
        town_name_temp = _route_description['town_name'] 
        route_id_temp = _route_description['id'] 
        self._town_name = town_name_temp
        self._route_id = route_id_temp
        # print("######***** {}".format(_route_description))                                                 
        ...
        scenario_type = ''
        if len(sampled_scenarios_definitions) > 0:
            scenario_type = sampled_scenarios_definitions[0]['name']
            print('####### scenario_type is {}'.format(scenario_type))
        else:
            rospy.logdebug('There is no sampled_scenarios !! ')
            return
        ...
        Common_Define.scenario_type = scenario_name[scenario_type]
        Common_Define.second_path = Common_Define.first_path + Common_Define.scenario_type + "/"
        isExist = os.path.exists( Common_Define.second_path )
        if not isExist:
            os.makedirs( Common_Define.second_path )
        now_time = datetime.datetime.now().strftime('%Y%m%d:%H%M%S')
        third_file_name = town_name_temp + "_" + "route" + route_id_temp +"_" + str(now_time)
        Common_Define.third_path = Common_Define.second_path + str(third_file_name) + "/"
        isThirdExist = os.path.exists(Common_Define.third_path)
        if not isThirdExist:
            os.makedirs(Common_Define.third_path)
        
        Common_Define.velocity_filename = Common_Define.third_path + 'velocity_file.txt'
        Common_Define.vehicle_location_filename = Common_Define.third_path + "vehicle_location_file.txt"
        Common_Define.ref_waypoint_filename = Common_Define.third_path + "ref_waypoint_file.txt"
        ...
        # print("################### {}".format(self.list_scenarios[-1][name]))
    ...
    def run(self, args):
        ...
        if not self.within_available_time():
            ...
            self.record_fatal_error(error_message)
            self._system_error = True
            rospy.logerr("#### Not enough simulation time available to continue ")
            sys.exit(-1)
        ...
        for repetition in range(self.repetitions):
            ...
            for route_idx, route_description in enumerate(route_descriptions_list):
                ...
                Common_Define.follow_speed = Common_Config.routes_follow_speed[config_num % len(route_descriptions_list) - 1]
                Common_Define.cruise_speed = Common_Config.routes_cruise_speed[config_num % len(route_descriptions_list) - 1]
                Common_Define.first_vehicle_location = Common_Config.routes_first_vehicle_location[config_num % len(route_descriptions_list) - 1]
                ...
                try:
                    ...
                    client.start_recorder('~/shared_dir/log_{}_track{}_route_{:0>4d}.log'.format(self.phase, self.track, route_idx))
                except:
                ...
                try:
                    self._current_route_broke = False
                    Common_Define.stop_update = False
...
def force_quit_handler(signum, frame):
    ...
    if Common_Define.save_excel :
        drawExcel = DrawExcel()
        drawExcel.write_excel_total()
        Common_Define.save_excel = False
