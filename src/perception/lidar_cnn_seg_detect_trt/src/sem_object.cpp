...
int SemObject::CommSemid(int proj_id, int nums, int flags)
{
    ...
    ROS_WARN_STREAM("[SEM] Key_t value is " << _key);
    if (_key>0)
    {   
        int ret = semget(_key, nums, flags);
        ROS_WARN_STREAM("[SEM] semget return value(SEM_ID) is " << ret);
...
{
    ROS_INFO("[SEM]sem destroyed entered already!");
    if (semctl(semid, 0, IPC_RMID)>0)       //IPC_RMID:删除信号量标识符
    {
        ROS_INFO_STREAM("[SEM]sem destroyed and ID is : " << semid);
        return 0;
    }
    else
    {
