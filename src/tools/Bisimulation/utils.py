...
class FrameStack(gym.Wrapper):
    ...
    def reset(self):
        ...
        for _ in range(self._k):
            self._frames.append(self.full_obs['birdeye'])
        return self._get_obs()[0], self._get_obs()[1]

    def step(self, action):
        self.full_obs, reward, done, info = self.env.step(action)
        self._frames.append(self.full_obs['birdeye'])
        return self._get_obs()[0], self._get_obs()[1], reward, done, info

    def _get_obs(self):
        assert len(self._frames) == self._k
        return np.concatenate(list(self._frames), axis=0), self.full_obs

    def updated_rendering_result(self, h_reconstruct, reconstruct_sample):
        self.env.updated_rendering_result(h_reconstruct, reconstruct_sample)
